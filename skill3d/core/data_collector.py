...
def _strip_time_fields(value: Any) -> Any:
    if isinstance(value, dict):
        return {
            key: _strip_time_fields(item)
            for key, item in value.items()
            if str(key).strip().lower() not in TIME_OUTPUT_KEYS
        }
    if isinstance(value, list):
        return [_strip_time_fields(item) for item in value]
    return value
...
class SessionData:
    ...
    def mark_success(self, final_answer: str):
        ...
        self.duration_seconds = round(time.monotonic() - self._start_monotonic, 6)
...
class DataCollector:
    ...
    def _save_session(self, session: SessionData) -> Dict[str, Any]:
        ...
        session_data = _strip_time_fields(session.to_dict())
        metadata_path = session_dir / "session_metadata.json"
        with open(metadata_path, 'w', encoding='utf-8') as f:
            json.dump(session_data, f, indent=2, ensure_ascii=False, default=_json_default)
        ...
        for i, sample in enumerate(session.samples):
            ...
            with open(sample_path, 'w', encoding='utf-8') as f:
                json.dump(_strip_time_fields(sample.to_dict()), f, indent=2, ensure_ascii=False, default=_json_default)
            ...
            if session_dir.is_dir():
                ...
                if metadata_path.exists():
                    with open(metadata_path, 'r', encoding='utf-8') as f:
                        session_data = _strip_time_fields(json.load(f))
                    
                    # Only include successful sessions
                    if session_data.get('success'):
                        all_samples.extend(_strip_time_fields(session_data.get('samples', [])))
        
        if os.environ.get("SPAGENT_FILTER_INVALID_SKILL3D_SFT", "0") == "1":
            before = len(all_samples)
            all_samples = [sample for sample in all_samples if self._is_skill3d_format_valid(sample)]
            logger.info("Filtered Skill-3D training export samples: %s -> %s", before, len(all_samples))

        # Export based on format
        if format == "jsonl":
            with open(output_path, 'w', encoding='utf-8') as f:
                for sample in all_samples:
                    f.write(json.dumps(_strip_time_fields(sample), ensure_ascii=False, default=_json_default) + '\n')
        
        elif format == "json":
            with open(output_path, 'w', encoding='utf-8') as f:
                json.dump(_strip_time_fields(all_samples), f, indent=2, ensure_ascii=False, default=_json_default)
            ...
            with open(output_path, 'w', encoding='utf-8') as f:
                json.dump(_strip_time_fields(sharegpt_data), f, indent=2, ensure_ascii=False, default=_json_default)
            ...
            if output_path.suffix == '.jsonl':
                with open(output_path, 'w', encoding='utf-8') as f:
                    for item in simple_data:
                        f.write(json.dumps(_strip_time_fields(item), ensure_ascii=False, default=_json_default) + '\n')
            else:
                with open(output_path, 'w', encoding='utf-8') as f:
                    json.dump(_strip_time_fields(simple_data), f, indent=2, ensure_ascii=False, default=_json_default)
    ...
    def _is_skill3d_format_valid(self, sample: Dict[str, Any]) -> bool:
        response = str(sample.get("response", "") or "")
        if "<think>" not in response or "</think>" not in response:
            return False
        if "<tool_choice>" in response or "</tool_choice>" in response:
            return False
        if "<tool_call>" in response and "<answer>" in response:
            return False
        for tag in ("think", "skill_choice", "tool_call", "answer"):
