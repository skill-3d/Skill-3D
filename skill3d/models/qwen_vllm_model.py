...
class QwenVLLMModel(Model):
    ...
    def __init__(
        ...
        if temperature is None:
            temperature = float(os.environ.get("SPAGENT_QWEN_TEMPERATURE", "0.1"))
        super().__init__(model_name, temperature, max_tokens)
        logger.info("Qwen VLLM sampling temperature: %s", temperature)
        
        # Import Qwen VLLM functions
        try:
            try:
                from skill3d.vllm_models.qwen_vllm import (
